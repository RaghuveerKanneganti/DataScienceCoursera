...
{ 
  final_df <- data.frame(hospital = character(), state = character())
  f <- read.csv("outcome-of-care-measures.csv", colClasses = "character")
  s <- unique(f$State)
  for (s1 in s)
  {
    if (!s1 %in% f$State) stop("invalid state")  
    ...
    final_df <- rbind(final_df, data.frame(hospital=hosp, state=s1))
  }
  rownames(final_df) <- final_df$state
  final_df <- final_df[order(rownames(final_df)), ]
}